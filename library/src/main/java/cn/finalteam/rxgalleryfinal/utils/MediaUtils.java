...
public class MediaUtils {
    ...
    public static List<MediaBean> getMediaAllList(Context context, String bucketId, int page, int limit) {
        int offset = (page - 1) * limit;
        List<MediaBean> mediaBeanList = new ArrayList<>();
        ContentResolver contentResolver = context.getContentResolver();
        List<String> projection = new ArrayList<>();
        ...
        if (!TextUtils.equals(bucketId, String.valueOf(Integer.MIN_VALUE))) {
            ...
            selectionArgs = new String[]{bucketId, String.valueOf(MediaStore.Files.FileColumns.MEDIA_TYPE_IMAGE),
        ...
        Cursor cursor = contentResolver.query(
                MediaStore.Files.getContentUri("external"), projection.toArray(new String[projection.size()]), selection,
    ...
    private static MediaBean parseVideoCursorAndCreateThumImage(Context context, Cursor cursor) {
        ...
        mediaBean.setVideoDuration(duration);
    ...
    public static List<BucketBean> getAllBucket(Context context, MediaType mediaType) {
        ...
        Cursor cursor = null;
        try {
            cursor = contentResolver.query(uri, projection, selection, selectionArgs, MediaStore.Video.Media.DATE_ADDED + " DESC");
        } catch (Exception e) {
            Logger.e(e);
        }
