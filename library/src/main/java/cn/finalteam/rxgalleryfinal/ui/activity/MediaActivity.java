 ...
 * Date:16/5/7 上午10:01
 */
public class MediaActivity extends BaseActivity implements ActivityFragmentView,IMediaActivityDelegate {
    ...
    private void subscribeEvent() {
        Subscription subscriptionOpenMediaPreviewEvent = RxBus.getDefault().toObservable(OpenMediaPreviewFragmentEvent.class)
                ...
                .subscribe(new RxBusSubscriber<OpenMediaPreviewFragmentEvent>() {
                    @Override
                    protected void onEvent(OpenMediaPreviewFragmentEvent openMediaPreviewFragmentEvent) throws Exception {
                        mPreviewPosition = 0;
                        showMediaPreviewFragment();
                    }
                });
    ...
    public MediaActivityDelegate getMediaActivityDelegate() {
        return new MediaActivityDelegate(this) {
            @Override
            protected TextView getOverActionView() {
                return mTvOverAction;
            }
            ...
            @Override
            public List<MediaBean> getCheckedList() {
                return MediaActivity.this.getCheckedList();
            }
        };
    }
}
